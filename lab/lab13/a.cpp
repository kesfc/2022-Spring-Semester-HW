...
public:
    // CONSTRUCTOR & DESTRUCTOR
    spBalloon(const std::string& name_) : name(name_) {
        std::cout << "Balloon constructor " << name << std::endl;
        num_ropes = 0;
        //ropes = new spBalloon*[MAX_NUM_ROPES];
    }
    ~spBalloon() {
        std::cout << "Balloon destructor " << name << std::endl;
        // don't try to destroy attached balloons, just delete array
    ...
    const std::string& getName() const { return name; }
    int numRopes() const { return num_ropes; }
    //const spBalloon* getRope(int i) const { return ropes[i]; }
    ...
    // add a rope connecting this balloon to another
    void addRope(dsSharedPtr<spBalloon>& b) {
        assert (num_ropes < MAX_NUM_ROPES);
        ropes[num_ropes] = b;
        num_ropes++;
    }
    // detach a rope connecting this balloon to another
    void removeRope(dsSharedPtr<spBalloon>& b) {
        for (int i = 0; i < MAX_NUM_ROPES; i++) {
            if (ropes[i] == b) { ropes[i] = ropes[num_ropes-1]; num_ropes--; break; }
        }
    }
...
private:
    ...
    int num_ropes;
    // a dynamically allocated C-style array of ropes connecting to other Balloons
    dsSharedPtr<spBalloon> ropes[MAX_NUM_ROPES];
};
...
//*  
    ...
    */
    ...
    dsSharedPtr<spBalloon> georgette(new spBalloon("Mr Potato Head"));
    dsSharedPtr<spBalloon> henry(new spBalloon("Snoopy"));
    georgette->addRope(henry);
    georgette->print();
    henry->print();

    dsSharedPtr<spBalloon> isabelle(new spBalloon("Shrek"));
    henry->addRope(isabelle);
    isabelle = dsSharedPtr<spBalloon>(new spBalloon("Barney the Purple Dinosaur"));
    //isabelle = tmp;
    georgette->addRope(isabelle);
...
void findAll(std::set<Balloon*>& a, Balloon* b){
  std::cout << "test" << std::endl;
  std::set<Balloon*>::iterator itr = a.end();
  for (int i = 0; i < b->numRopes(); i++){
    unsigned int org = a.size();
    a.insert(b->getRope(i));
    if (a.size() > org)
      findAll(a, b->getRope(i));
  }
}
...
void deleteAll(Balloon* b){
  std::set<Balloon*> all;
  findAll(all, b);
  std::set<Balloon*>::iterator itr = all.begin();
  while (itr != all.end()){
      std::set<Balloon*>::iterator next = itr;
      delete *itr;
      next++;
      itr = next;
  }
