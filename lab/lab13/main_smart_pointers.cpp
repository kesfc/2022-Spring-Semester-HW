...
/*class SH_ballom{
  public:
    SH_ballom(const std::string& name_) : name(name_) {
      std::cout << "Balloon constructor " << name << std::endl;
      num_ropes = 0;
    }
  ~SH_ballom() {
  ...
  // add a rope connecting this balloon to another
  void addRope(dsSharedPtr<SH_ballom> b) { 
    assert (num_ropes < MAX_NUM_ROPES);
    ropes[num_ropes] = b;
    num_ropes++;
  }
  ...
  // detach a rope connecting this balloon to another
  void removeRope(dsSharedPtr<SH_ballom> b) { 
    for (int i = 0; i < MAX_NUM_ROPES; i++) {
      if (ropes[i] == b) { ropes[i] = ropes[num_ropes-1]; num_ropes--; break; }
    }
  }
private:
  ...
  int num_ropes;
  // a dynamically allocated C-style array of ropes connecting to other Balloons
  dsSharedPtr<SH_ballom> ropes[MAX_NUM_ROPES]; 
};
*/
void deleteAll(Balloon* root){
    ...
    while (true){
        int old_size = bs.size();
        std::vector<Balloon*> next_gen;
        for (std::set<Balloon*>::iterator it = bs.begin(); it != bs.end(); it++){
            for (int i = 0; i < (*it)->numRopes(); i++){
                next_gen.push_back((*it)->getRope(i));
            }
        }


        for (int i = 0; i < (int)next_gen.size(); i++){
            bs.insert(next_gen[i]);
        }

        if ((int)bs.size() == old_size){
            break;
        }
    }
  ...
  /*void find(Balloon *jacob, std::set<Balloon*>& all){
    if(jacob==NULL){
      return;
    }
    else{
      all.insert(jacob);{
        ...
        for(int i=0; i<*jacob->numRopes(); i++){
          find(((*jacob)->getRope(i)),all);
        }
      }
    }
  }
  void deleteAll(Balloon* jacob){
      std::set<Balloon*> all;
      find(jacob,all);
      std::set<Balloon*>::iterator a;
...
/*
  ...
  dsSharedPtr<SH_ballom> georgette(new SH_ballom("Mr Potato Head"));
  dsSharedPtr<SH_ballom> henry(new SH_ballom("Snoopy"));
  georgette->addRope(henry);
  georgette->print();
  henry->print();
  henry=dsSharedPtr<SH_ballom> (new SH_ballom("Tigger"));
  georgette->addRope(henry);
  dsSharedPtr<SH_ballom> isabelle(new SH_ballom("Shrek"));
  henry->addRope(isabelle);
  isabelle=dsSharedPtr<SH_ballom>(new SH_ballom("Barney the Purple Dinosaur"));
  georgette->addRope(isabelle);
  henry->print();
  georgette->print();
  isabelle->print();
