...
void vector_longest_substring(std::vector<std::string> &vec, std::ostream &ostr, 
int &output_count){
  std::string Final;
  for (unsigned int i = 0; i < vec.size(); i++){
    for (unsigned int k = 0; k < vec[i].size(); k++){
      for (unsigned j = k+1; j < vec[i].size(); j++){
        std::string temp = vec[i].substr(k, j - k);//find all possible substring
